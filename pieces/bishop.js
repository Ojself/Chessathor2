class Bishop {
  ...
  setup() {
    this.bishopImg = loadImage('assets//wb.png');
  }
  ...
  collisionCheck(player) {
    if (this.dead || player.isMoving) {
      return false;
    }

    const downRight = checkCollidingDiagonal(
      player,
      this.x,
      this.y,
      1,
    ...
    const upRight = checkCollidingDiagonal(
      player,
      this.x,
      this.y,
      1,
    ...
    const downLeft = checkCollidingDiagonal(
      player,
      this.x,
      this.y,
      -1,
    ...
    const upLeft = checkCollidingDiagonal(
      player,
      this.x,
      this.y,
      -1,
...
function checkCollidingDiagonal(player, x, y, dirX, dirY, id) {
  for (
    let i = 0;
    i < 8 && checkCollidingWithBishop(x, y, i, dirX, dirY, id);
    i++
  ) {
    ...
    if (
      player.x + player.width > helperX &&
      player.x - player.width < helperX
    ) {
      if (
        player.y + player.width > helperY &&
        player.y - player.height < helperY
      ) {
        return true;
      }
    }
...
function checkCollidingWithBishop(x, y, i, dirX, dirY, id) {
  ...
  if (gamePieces.find((p) => p.x === helperX && p.y === helperY)) {
    return false;
  }
  return true;
}
