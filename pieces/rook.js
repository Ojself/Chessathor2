class Rook {
  ...
  setup() {
    this.rookImg = loadImage('assets/wr.png');
  }
  ...
  collisionCheck(player) {
    ...
    const down = checkCollidingStraight(
      player,
      this.x,
      this.y,
      -1,
    ...
    const left = checkCollidingStraight(
      player,
      this.x,
      this.y,
      1,
    ...
    const right = checkCollidingStraight(
      player,
      this.x,
      this.y,
      -1,
...
function checkCollidingStraight(player, x, y, dir, vertical, id) {
  for (
    let i = 0;
    i < 8 && checkCollidingWithRook(x, y, i, dir, vertical, id);
    i++
  ) {
    const verticalValues = [x, y + 100 * dir + i * 100 * dir, 50, 50];
    const horizontalValues = [x + 100 * dir + i * 100 * dir, y, 50, 50];

    const valueHelper = vertical
      ? [verticalValues[0], verticalValues[1]]
      : [horizontalValues[0], horizontalValues[1]];

    if (
      player.x + player.width > valueHelper[0] &&
      player.x - player.width < valueHelper[0] &&
      player.isMoving === false
    ) {
      if (
        player.y + player.height >= valueHelper[1] &&
        player.y - player.height <= valueHelper[1]
      ) {
        return true;
      }
...
function checkCollidingWithRook(x, y, i, dir, vertical, id) {
  ...
  if (vertical) {
    gamePieces = game.pieces
      .filter((p) => p.x === x)
      .filter((p) => p.id !== id);
    if (gamePieces.find((p) => p.y === y + i * 100 * dir)) {
      return false;
    }
  } else {
    gamePieces = game.pieces
      .filter((p) => p.y === y)
      .filter((p) => p.id !== id);
    if (gamePieces.find((p) => p.x === x + i * 100 * dir)) {
      return false;
    }
  }
  return true;
